...
fbref_1992_2016 <- read_csv('data/fbref 1992 to 2016.csv') %>%
...
### Collate 2017/18 - 2023/24 data into one df
## Combine stat types into one df
all_2017_2023 <- standard %>% 
...
write_excel_csv(all_2017_2023,'data/2017 to 2023 raw.csv')
all_2017_2023 <- read_csv('data/2017 to 2023 raw.csv')


#### 4) Exporting datasets from scraped data

## Creating seasons dataset for 2017/18 - 2023/24 (combining any rows where a player has played for more than one team)
all_2017_2023_seasons <- all_2017_2023 %>%
  group_by(Player,player_id,Season_End_Year,Nation,Born,Age,Url) %>%
  summarise(across(where(is.numeric), sum), .groups = 'drop') %>% 
  left_join(all_2016_2024 %>%
              modal_attributes(season = TRUE)) 

## Creating season dataset for 2023/24 (combining any rows where a player has played for more than one team)
all_2023_24_season <- all_2017_2023_seasons %>%
  filter(Season_End_Year == 2024)


## Combining 1992/93 - 2016/17 and 2017/18 - 2023/24 datasets together
all_seasons <- bind_rows(all_2017_2023_seasons,fbref_1992_2017_seasons) %>%
  group_by(Player,Born) %>%
  mutate(player_id = cur_group_id()) %>% 
  ungroup() %>%
  select(-Days)
