...
all_summed <- all_seasons %>% select(Player,player_id,Position,Squad,Comp,Nation,Born,Age,Matches,Starts,Minutes,G,NPG,A,PK,PKatt,CrdY,CrdR) %>%
  ...
  summarise(across(c(Minutes,G,NPG,A,PK,PKatt,CrdY,CrdR), sum, na.rm = TRUE), .groups = 'drop') %>%
...
ggplot(all_per90_display %>% mutate(color =  ifelse(is.na(Display), "Grey", Position)) %>%
  ...
  football_theme() +
  guides(size = 'none',
         col = 'none') -> plot_9224_xa_npg
ggsave("viz/9224 a vs npg.pdf", plot_9224_xa_npg, width = 25, height = 29,
       bg = 'transparent')
